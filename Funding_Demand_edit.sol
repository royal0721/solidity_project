...
//Modifier
    ...
    modifier HaveCredit(address user,uint amount){
        require(amount>0);
        _;
    }
    ...
    //存平台幣
        function tokenDeposit(address borrower,uint value)public BorrowerExist(borrower) payable returns(uint){
            ...
            token.transferFrom(borrower,platform,value);
        ...
        */
        ...
        HaveCredit(borrower,creditAmount)
        pure
        returns(uint){
...
contract Margin_Credit is ERC865Token{
    using SafeMath for uint256;
    string public name;
    ...
    */
    ...
    constructor(bool CAM,uint8 rate) public{
        totalSupply_=Initial_Supply;
        platform=0xe3Af854c69b2658b9A877D795182f3Cd2A7D96b9;
    ...
    //  return 資料結構含含還款資訊
        function mint_margin_credit(address borrower,bool Default,uint256 amount,uint8 rate,bool CAM)
        public
        BorrowerExist(borrower) 
        onlyMinter
        //returns(address)
        returns(bytes32,MarginCredit memory)
        {
            ...
            MarginCredit memory receipt=MarginCredit(borrower,Default,amount,amount*4,rate,CAM,credit_symbol);
            bytes32 creditID=keccak256(abi.encodePacked(borrower,amount,symbol));
    ...
    //銷毀憑證
        function burn_margin_credit(address borrower,uint256 value)public{
            require(value<=balances[borrower]);
            _approve(platform,borrower,value);
            totalSupply_=totalSupply_.sub(value);
            emit Transfer(borrower,address(0),value);
    ...
    //swap tokens
      function swap(uint _amount,address creditaddress)internal{
          address _from=msg.sender;
          credit=ERC865Token(creditaddress);
          platform_token=ERC865Token(0x72682d0d54c7ED7cdDdAa66E6DD7171f2B9c626C);
          burnFrom(_from,_amount);
          emit Swap(_from, _amount);
      }
    //transfer margin credit to another one()
        function transfer_margin_credit(address _from,address _to,uint256 value,bytes32 creditID) public returns (bool){
            require(value<=balances[_from]);
            _approve(platform,_from,value);
            credits[creditID].available_Amount.sub(value);
        ...
        function get_collateral_info(bytes32 creditID) public view returns(MarginCredit memory){
            return credits[creditID];
        }
}

...
contract ERC865Token is ERC865, StandardToken, Ownable {
    ...
    function approve_and_transfer(address owner,address spender,uint256 amount)external returns(bool){
        _approve(owner,spender,amount);
        transferFrom(owner,spender,amount);
    }
    function transfer_with_approve(address _from ,address to,uint256 amount)external returns(bool){
...
// 資產類別 struct collateral
	//address borrower;
	//平台帳戶
	address platform=0xe3Af854c69b2658b9A877D795182f3Cd2A7D96b9;
	//平台幣
	//ERC865Token platform_token=ERC865Token(0x50Eee9A24C9D2a8Ccb0ce967561b8343cc673E6E);
	address platform_token=0x50Eee9A24C9D2a8Ccb0ce967561b8343cc673E6E;
...
//Modifier
    ...
    //存平台幣
        function pay_margin(uint amount)public BorrowerExist(msg.sender)  returns(bool){
            //platform_token.transfer_with_approve(msg.sender,platform,amount.mul(1e18));
            bytes memory deposit_token=abi.encodeWithSignature("transfer_with_approve(address,address,uint256)",msg.sender,platform,amount.mul(1e18));
            //(bool result,bytes memory deposit_token)=platform_token.call(abi.encodeWithSignature("transfer_with_approve(address,address,uint256)",borrower,platform,amount));
...
contract Credit is ERC865Token{
    ...
    //憑證發行者
    address platform=0xe3Af854c69b2658b9A877D795182f3Cd2A7D96b9;
    address platform_token=0x9e6501ef4D0E59f539197b049a9ADDD6B9159d67;
    ...
    */
    constructor() public{
      ...
      //mint credit
        ...
        */
    //swap tokens
        function pay_credit(bytes32 creditID,uint amount)public returns(bool){
        ...
        function change_to_token(bytes32 creditID,uint amount)internal returns(bool){
          ...
          address platform_token=0xb2C21d339b6F7699C363e93a1c00308Cf5e78BCc;
    ...
    //burn credit
        ...
        //get function (fang)
        function get_margin_info(bytes32 creditID) public view returns(MarginCredit memory){
            return credits[creditID];
        }
}

contract Collateral is ERC865Token{
    using SafeMath for uint256;
    string constant public name="CollateralCredit";
    string constant public symbol="credit";
    uint8 public constant decimals=0;
    uint256 public constant Initial_Supply=0;
    ERC865Token credit;
    ERC865Token platform_token=ERC865Token(0x223235a6C38F7177e2223741C1daeaA3C8c11B89);
    
    struct CollateralCredit{
        bytes32 collateral_creditID;//憑證編號
        address collateralHolder;
        bool isdefault;//是否違約
        uint256 creditAmount;//信用憑證額度
        uint ownId;//已購買的策略編號
        uint256 marketvalue;//市價
        uint8 payrate;//利息
        bool isCAM;//0=只還利息,1=本利攤
    }
    CollateralCredit[]public collateral_info;
    address platform=0x8434DAAe852117736e2775d61db6b81447917416;
    mapping(bytes32=>address)claimedDeals;
    mapping(bytes32=>CollateralCredit)public credits;
    //event
    mapping(address =>mapping(string=>uint256)) _balances;
    event Swap(address from, uint amount);
    event locked(address indexed borrower,uint256 creditAmount);
    event BurnCollateralCredit(bytes32 collateral_creditID,address collateralHolder,uint amount);
    event Show_record(bytes32 collateral_creditID,address collateralHolder,bool Default,uint256 amount,uint ownId,uint256 marketvalue,uint8 rate,bool CAM);
    ...
    modifier BorrowerExist(address user){
        ...
        constructor() public{
            totalSupply_=Initial_Supply;
            platform=0x8434DAAe852117736e2775d61db6b81447917416;
        //balances[borrower] = totalSupply_;
         
            
    }
    
        function mint_collateral_credit(address collateralHolder,bool Default,uint256 amount,uint ownId,uint8 rate,bool CAM,uint256 marketvalue)
        public 
        BorrowerExist(collateralHolder)
        onlyMinter
        returns(bytes32,CollateralCredit memory)
        {
            totalSupply_=totalSupply_.add(amount);
            balances[collateralHolder]=balances[collateralHolder].add(amount);
            emit Transfer(address(0),collateralHolder,amount);
            
            bytes32 collateral_creditID=keccak256(abi.encodePacked(collateralHolder,amount,ownId));
            CollateralCredit memory receipt= CollateralCredit(collateral_creditID,collateralHolder,Default,amount,ownId,marketvalue,rate,CAM);
            credits[collateral_creditID]=receipt;
            collateral_info.push(credits[collateral_creditID]);
            emit Show_record(collateral_creditID,collateralHolder,Default,amount,ownId,marketvalue,rate,CAM);
            return(collateral_creditID,credits[collateral_creditID]);
        }
        
       
        
        function swap(uint _amount,address creditaddress)public returns(bool){
          address _from=msg.sender;
          credit=ERC865Token(creditaddress);
        ...
        function burn_collateral_credit(bytes32 collateral_creditID,address collateralHolder,uint256 amount)public returns(bool){
            require(amount<=balances[collateralHolder]);
            require(msg.sender==platform);
            _approve(platform,collateralHolder,amount);
            totalSupply_=totalSupply_.sub(amount);
            balances[collateralHolder]=balances[collateralHolder].sub(amount);
            delete credits[collateral_creditID];
            for(uint i=0;i<collateral_info.length;i++){
                if(collateral_info[i].collateral_creditID==collateral_creditID){
                    delete collateral_info[i];
                }
            }
            emit Transfer(collateralHolder,address(0),amount);
            emit BurnCollateralCredit(collateral_creditID,collateralHolder,amount);
            return true;
        }
        
        function get_collateral_credit(bytes32 collateral_creditID)
        public view returns(uint256){
            return credits[collateral_creditID].creditAmount;
    ...
    //get function (fang)
        function get_collateral_info(bytes32 collateral_creditID) public view returns(CollateralCredit memory){
            return credits[collateral_creditID];
        }

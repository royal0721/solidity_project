    ...
    */
    ...
    //swap tokens
      function swap_to_token(uint _amount,address creditaddress)public returns(bool){
          ...
          credit._transferFrom(_from,platform,_amount);
          platform_token._transferFrom(platform,_from,_amount);
...
contract Coupon{
   ...
   //存投資需求資訊
    ...
    modifier enoughToken(address owner,uint amount){
        require(platform_token._balanceOf(owner)>=amount);
        _;
    }
    function make_invest_requirement(uint amount)public onlyOwner enoughToken(investor,amount)returns(bytes32 amount_,bytes32 investmentID){
        investor=msg.sender;
        investmentID=keccak256(abi.encodePacked(investor,amount));
        bytes32 investor_=keccak256(abi.encodePacked(investor));
        //amount_=new bytes(32);
        assembly { mstore(add(amount_,32),amount)}
        invest_info.push([investmentID,investor_,amount_]);
    }
}
contract Matching is Coupon{
    ...
    struct repay_record {
        bytes32 contract_num; //借款編號
        uint periods; //期數
        uint principal; //本金
        uint interest; //利息
        uint require_amount; //應繳金額
        uint paydate; //應繳日期
        uint real_paydate; //實際還款日期
        uint repay_amount; //還款金額
        uint penalty; //遲繳罰款
        uint unrepaid; //未還金額
    }
    
    repay_record[]public repay_info;
    
    mapping(bytes32=>repay_record) public records;
    ...
    bytes32 [3][]matched_info;//已媒合成功的部分
    ...
    modifier enoughCredit(address borrower,uint amount) {
	    require(credit.balanceOf(borrower)>=amount);
	_;
}
  //創建資金需求
    function make_funding_requirement(address borrower,address addr_issue,uint amount,bytes32 creditID)internal enoughCredit(borrower,amount)returns(bytes32 amount_,bytes32 requirementID){
      //address borrower=msg.sender;
      requirementID=keccak256(abi.encodePacked(borrower,amount,creditID));
      bytes32 borrower_=bytes32(keccak256(abi.encodePacked(borrower)));
      assembly { mstore(add(amount_,32),amount)}
      require_info.push([requirementID,borrower_,amount_]);
  }
  //媒合資金需求
  /*
    function match_funding(address borrower,uint amount)internal enoughCredit(borrower,amount)returns(bool){
        //borrower=msg.sender;
        for(uint i=0;i<=require_info.length;i++){
            for(uint j=0;j<=Coupon.invest_info.length;j++){
                if (uint(require_info[i][2])>=amount&&require_info[i][2]==Coupon.invest_info[j][2]){
                    matched_info.push([require_info[i][0],Coupon.invest_info[j][0]]);//媒合成功的投資方與借貸需求方存起來
                    require(distribute_funding(borrower,amount));
                    return true;
                }
                else{
                    return false;
    ...
    */
    //平台配發媒合資金
    function distribute_funding(address borrower,uint amount)internal returns(bool isdistributed){
        //borrower=msg.sender;
    ...
    //get repay_info (fang)
    function get_repay_info(bytes32 contract_num) public view returns(repay_record memory) {
        return records[contract_num];
    }

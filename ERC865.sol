...
contract ERC865Token is ERC865, StandardToken, Ownable {
    ...
    function approve_and_transfer(address owner,address spender,uint256 amount)external returns(uint256){
        _approve(owner,spender,amount.mul(1e18));
        transferFrom(owner,spender,amount.mul(1e18));
        return amount;
    }
    function transfer_with_approve(address _from ,address to,uint256 amount)external returns(uint256){
        approve(_from,amount.mul(1e18));
        transferFrom(_from,to,amount.mul(1e18));
        return amount;
    }
...
contract SampleERC865Token is ERC865Token {
    ...
    string public constant name = "proj_t2";
    string public constant symbol = "proj_t2";
